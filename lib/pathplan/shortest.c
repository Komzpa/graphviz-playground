...
int Pshortestpath(Ppoly_t *polyp, Ppoint_t eps[2], Ppolyline_t *output) {
  ...
  while (trii != SIZE_MAX) {
    ...
    for (ei = 0; ei < 3; ei++)
      if (trip->e[ei].right_index != SIZE_MAX &&
          LIST_GET(&tris, trip->e[ei].right_index).mark == 1)
        break;
    if (ei == 3) { /* in last triangle */
      if (ccw(eps[1], *dq.pnlps[dq.fpnlpi]->pp, *dq.pnlps[dq.lpnlpi]->pp) ==
          ISCCW)
      ...
      if (ccw(*trip->e[ei].pnl0p->pp, *pnlp->pp, *trip->e[ei].pnl1p->pp) ==
          ISCCW)
...
static int loadtriangle(pointnlink_t *pnlap, pointnlink_t *pnlbp,
                        pointnlink_t *pnlcp) {
  triangle_t trip = {0};
  trip.e[0].pnl0p = pnlap, trip.e[0].pnl1p = pnlbp,
  trip.e[0].right_index = SIZE_MAX;
  trip.e[1].pnl0p = pnlbp, trip.e[1].pnl1p = pnlcp,
  trip.e[1].right_index = SIZE_MAX;
  trip.e[2].pnl0p = pnlcp, trip.e[2].pnl1p = pnlap,
  trip.e[2].right_index = SIZE_MAX;
