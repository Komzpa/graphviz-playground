...
static void clear_flat_matrices(graph_t *g);
...
void checkLabelOrder(graph_t *g) {
  ...
  for (int r = GD_minrank(g); r <= GD_maxrank(g); r++) {
    ...
    for (int j = 0; j < rk->n; j++) {
      ...
      if (ND_alg(u)) {
        if (!lg)
          lg = agopen("lg", Agstrictdirected, 0);
...
/* Preserve the input order of the nodes in a graph or subgraph, rank by rank.
...
static void install_rankorder(graph_t *g) {
  ...
  const int rank_count = GD_maxrank(Root) - minrank + 1;
  node_t **const previous = gv_calloc((size_t)rank_count, sizeof(*previous));

  /* agfstnode() walks the subgraph's node sequence in input (AGSEQ) order. */
  for (node_t *n = agfstnode(g); n; n = agnxtnode(g, n)) {
    const int rank = ND_rank(n);
    if (rank < minrank || rank >= minrank + rank_count)
      continue;
...
static void do_rankorder_for_subgraphs(graph_t *g) {
  do_rankorder(g);
  for (graph_t *subg = agfstsubg(g); subg; subg = agnxtsubg(subg)) {
    /* clusters are processed by their own mincross invocation */
    if (!is_a_cluster(subg))
      do_rankorder_for_subgraphs(subg);
  }
}
...
static int64_t mincross(graph_t *g, int startpass) {
  ...
  for (pass = startpass; pass <= endpass; pass++) {
    ...
    for (iter = 0; iter < maxthispass; iter++) {
      if (Verbose)
        fprintf(stderr,
                "mincross: pass %d iter %d trying %d cur_cross %" PRId64
                " best_cross %" PRId64 "\n",
...
static void merge2(graph_t *g) {
  ...
  for (r = GD_minrank(g); r <= GD_maxrank(g); r++) {
    ...
    for (i = 0; i < GD_rank(g)[r].n; i++) {
      ...
      if (v == NULL) {
        if (Verbose)
          fprintf(stderr, "merge2: graph %s, rank %d has only %d < %d nodes\n",
...
static bool is_rankorder_edge(edge_t *e) {
  return ED_edge_type(e) == FLATORDER && ED_alg(e) == &rankorder_marker;
}
    ...
    for (i = 0; (e = ND_flat_out(v).list[i]); i++) {
      ...
      if (ND_onstack(aghead(e))) {
        matrix_set(M, (size_t)flatindex(aghead(e)),
                   (size_t)flatindex(agtail(e)));
        if (is_rankorder_edge(e)) {
          edge_t *const rev = find_flat_edge(aghead(e), agtail(e));
          if (rev && !is_rankorder_edge(rev)) {
            delete_flat_edge(rev);
            flat_rev(g, rev);
            continue;
          }
        }
        ...
        matrix_set(M, (size_t)flatindex(agtail(e)),
                   (size_t)flatindex(aghead(e)));
        if (!ND_mark(aghead(e)))
...
void allocate_ranks(graph_t *g) {
  ...
  int *cn = gv_calloc(GD_maxrank(g) + 2,
                      sizeof(int)); // must be 0 based, not GD_minrank
  for (n = agfstnode(g); n; n = agnxtnode(g, n)) {
    cn[ND_rank(n)]++;
    for (e = agfstout(g, n); e; e = agnxtout(g, e)) {
...
int install_in_rank(graph_t *g, node_t *n) {
  ...
  if (ND_order(n) > GD_rank(Root)[r].an) {
    agerrorf("install_in_rank, line %d: ND_order(%s) [%d] > "
             "GD_rank(Root)[%d].an [%d]\n",
  ...
  if (GD_rank(g)[r].v + ND_order(n) > GD_rank(g)[r].av + GD_rank(Root)[r].an) {
    agerrorf("install_in_rank, line %d: GD_rank(g)[%d].v + ND_order(%s) [%d] > "
             "GD_rank(g)[%d].av + GD_rank(Root)[%d].an [%d]\n",
