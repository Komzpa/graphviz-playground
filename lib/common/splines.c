...
static int debugleveln(edge_t *e, int i) {
  return GD_showboxes(agraphof(aghead(e))) == i ||
         GD_showboxes(agraphof(agtail(e))) == i || ED_showboxes(e) == i ||
         ND_showboxes(aghead(e)) == i || ND_showboxes(agtail(e)) == i;
}
 ...
 */
static void arrow_clip(edge_t *fe, node_t *hn, pointf *ps, size_t *startp,
                       size_t *endp, bezier *spl, splineInfo *info) {
  edge_t *e;
  bool j;
  ...
  if (info->isOrtho) {
    ...
    if (sflag)
      *startp =
          arrowStartClip(e, ps, *startp, *endp, spl, sflag, start_arrowsize);
    if (eflag)
      *endp = arrowEndClip(e, ps, *startp, *endp, spl, eflag, end_arrowsize);
  }
}
...
void bezier_clip(inside_t *inside_context,
                 bool (*inside)(inside_t *inside_context, pointf p), pointf *sp,
                 bool left_inside) {
 ...
 */
static void shape_clip0(inside_t *inside_context, node_t *n, pointf curve[4],
                        bool left_inside) {
  int i;
 ...
 * edge.
 */
void clip_and_install(edge_t *fe, node_t *hn, pointf *ps, size_t pn,
                      splineInfo *info) {
  ...
  if (!info->ignoreSwap && ND_rank(tn) == ND_rank(hn) &&
      ND_order(tn) > ND_order(hn)) {
    SWAP(&hn, &tn);
  }
  if (tn == agtail(orig)) {
...
static double conc_slope(node_t *n) {
  ...
  if (cnt_in == 0)
    return cnt_out > 0 ? m_out : M_PI / 2.0;
  if (cnt_out == 0)
    return m_in;
...
void beginpath(path *P, edge_t *e, int et, pathend_t *endp, bool merge) {
  ...
  P->data = e;
  endp->np = P->start.p;
  if (et == REGULAREDGE && ND_node_type(n) == NORMAL &&
      (side = ED_tail_port(e).side)) {
    edge_t *orig;
    boxf b0, b = endp->nb;
    if (side & TOP) {
      endp->sidemask = TOP;
  ...
  if (et == REGULAREDGE)
    ...
    side = endp->sidemask; /* for flat edges */
  if (pboxfn &&
...
static void selfBottom(edge_t *edges[], size_t cnt, double sizex, double stepy,
  ...
  dy = ND_ht(n) / 2.0;
  dx = 0.0;
  // certain adjustments are required for some point_pairs in order to improve
  // the display of the edge path between them
  point_pair =
      convert_sides_to_points(ED_tail_port(e).side, ED_head_port(e).side);
  switch (point_pair) {
  case 67:
    sgn = -sgn;
    break;
...
static void selfTop(edge_t *edges[], size_t cnt, double sizex, double stepy,
  ...
  case 83:
    ...
    break;
  case 84:
    dx = sgn * ((ND_lw(n) - (np.x - tp.x) + (ND_rw(n) - (hp.x - np.x))) / 2.0 +
                stepx);
    break;
  case 74:
  case 75:
...
double selfRightSpace(edge_t *e) {
  ...
  if ((!ED_tail_port(e).defined && !ED_head_port(e).defined) ||
      (!(ED_tail_port(e).side & LEFT) && !(ED_head_port(e).side & LEFT) &&
...
void makePortLabels(edge_t *e) {
   ...
   */
  if ((!E_labelangle || agxget(e, E_labelangle)[0] == '\0') &&
      (!E_labeldistance || agxget(e, E_labeldistance)[0] == '\0'))
    return;
