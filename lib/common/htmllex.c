...
static void error_context(htmllexstate_t *ctx) {
  agerr(AGPREV, "... %.*s%.*s ...\n", (int)ctx->prevtok.size, ctx->prevtok.data,
        (int)ctx->currtok.size, ctx->currtok.data);
}
...
static int stylefn(htmldata_t *p, const char *v) {
  ...
  for (tok_t t = tok(v, DELIM); !tok_end(&t); tok_next(&t)) {
    ...
    if (strview_case_str_eq(tk, "ROUNDED"))
      p->style.rounded = true;
    else if (strview_case_str_eq(tk, "RADIAL"))
      p->style.radial = true;
    else if (strview_case_str_eq(tk, "SOLID")) {
      p->style.dotted = false;
      p->style.dashed = false;
    } else if (strview_case_str_eq(tk, "INVISIBLE") ||
               strview_case_str_eq(tk, "INVIS"))
      p->style.invisible = true;
    else if (strview_case_str_eq(tk, "DOTTED"))
      p->style.dotted = true;
    else if (strview_case_str_eq(tk, "DASHED"))
      p->style.dashed = true;
    else {
...
static int cellborderfn(htmldata_t *data, const char *v) {
  long u;
  htmltbl_t *const p =
      (htmltbl_t *)((uintptr_t)data - offsetof(htmltbl_t, data));
...
static int rowspanfn(htmldata_t *data, const char *v) {
  long u;
  htmlcell_t *const p =
      (htmlcell_t *)((uintptr_t)data - offsetof(htmlcell_t, data));
...
typedef struct {
  ...
  int (*action)(htmldata_t *,
                const char *); ///< action to perform if name matches
} html_item_t;
...
#define CALL_ACTION(list, elem, tp, val)                                       \
  (_Generic((list),                                                            \
       const html_item_t *: (const html_item_t *)(elem),                       \
       const font_item_t *: (const font_item_t *)(elem),                       \
       const img_item_t *: (const img_item_t *)(elem),                         \
       const br_item_t *: (const br_item_t *)(elem))                           \
       ->action((tp), (val)))
 ...
 */
#define doAttrs(ctx, tp, items, nel, atts, s)                                  \
  do {                                                                         \
    const char *name;                                                          \
                                                                               \
    while ((name = *(atts)++) != NULL) {                                       \
      const char *val = *(atts)++;                                             \
      void *const ip =                                                         \
          bsearch(name, (items), (nel), sizeof((items)[0]), icmp);             \
      if (ip)                                                                  \
...
static htmlimg_t *mkImg(htmllexstate_t *ctx, const char **atts) {
  ...
  doAttrs(ctx, img, img_items, sizeof(img_items) / sizeof(img_items[0]), atts,
          "<IMG>");
...
static textfont_t *mkFont(htmllexstate_t *ctx, const char **atts,
  ...
  if (atts)
    doAttrs(ctx, &tf, font_items, sizeof(font_items) / sizeof(font_items[0]),
            atts, "<FONT>");
...
static htmlcell_t *mkCell(htmllexstate_t *ctx, const char **atts) {
  ...
  cell->colspan = 1;
  cell->rowspan = 1;
  doAttrs(ctx, &cell->data, cell_items,
          sizeof(cell_items) / sizeof(cell_items[0]), atts, "<TD>");
...
static htmltbl_t *mkTbl(htmllexstate_t *ctx, const char **atts) {
  ...
  doAttrs(ctx, &tbl->data, tbl_items, sizeof(tbl_items) / sizeof(tbl_items[0]),
          atts, "<TABLE>");
...
int initHTMLlexer(htmlscan_t *scanner, char *src, agxbuf *xb, htmlenv_t *env) {
  ...
  if (!atomic_flag_test_and_set(&first)) {
    agwarningf("Not built with libexpat. Table formatting is not available.\n");
  }
  return 1;
#endif
}
