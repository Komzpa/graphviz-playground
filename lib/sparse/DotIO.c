...
static void color_string(agxbuf *buf, int dim, double *color) {
  ...
  if (dim == 3) {
    ...
    agxbprint(buf, "#%02x%02x%02x", MIN((unsigned int)(color[0] * 255), 255), 0,
              MIN((unsigned int)(color[1] * 255), 255));
  }
}
...
int Import_dot_splines(Agraph_t *g, int *ne, char ***xsplines) {
  /* get the list of splines for the edges in the order they appear, and store
     as a list of strings in xspline. If *xsplines = NULL, it will be allocated.
     On exit (*xsplines)[i] is the control point string for the i-th edge. This
     string is of the form "x1,y1 x2,y2...", the two end points of the edge is
     not included per Dot format Return 1 if success. 0 if not.
  */
  Agnode_t *n;
  Agedge_t *e;
  Agsym_t *sym;
...
SparseMatrix Import_coord_clusters_from_dot(
  ...
  *clusters = gv_calloc(nnodes, sizeof(int));
   ...
   * to clusters 2, 3, .... Any nodes not in a cluster subgraph are tossed into
   * cluster 1.
   */
  if (useClusters) {
    Agraph_t *sg;
    ...
    for (sg = agfstsubg(g); sg; sg = agnxtsubg(sg)) {
      if (!startswith(agnameof(sg), "cluster"))
        continue;
      gid++;
  ...
  if (noclusterinfo) {
    ...
    if (clustering_scheme == CLUSTERING_MQ) {
      ...
      modularity_clustering(A, false, maxcluster, &nc, clusters, &modularity);
    } else {
      UNREACHABLE();
    }
    for (i = 0; i < nnodes; i++)
      (*clusters)[i]++; /* make into 1 based */
    for (n = agfstnode(g); n; n = agnxtnode(g, n)) {
      i = ND_id(n);
