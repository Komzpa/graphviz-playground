#!/usr/bin/env python3
"""Verify curved-node arrow spreading without spline kinks.
...
def target_overlap_area(layouts: list[dict[str, Any]]) -> float:
    heads = []
    for layout in layouts:
        for edge in layout.get("edges", []):
            poly = polygon_points(edge, "_hdraw_")
            if poly is not None:
                heads.append((edge.get("head"), poly))
    total = 0.0
    for i, (head, left) in enumerate(heads):
        for other_head, right in heads[i + 1 :]:
            if other_head == head:
...
def main() -> int:
    ...
    items = [(binary, mode, rel) for binary in ("base", "current") for mode in ("default", "false", "true") for rel in rels]
    dots = {"base": base_dot, "current": dot}
    renders: dict[tuple[str, str, str], Render] = {}
    with futures.ThreadPoolExecutor(max_workers=JOBS) as pool:
        future_map = {
            pool.submit(render, dots[binary], root, rel, mode): (binary, mode, rel)
    ...
    for rel in rels:
        ...
        if comparable(before_true, after_true) and before_true.xdot != after_true.xdot:
            if nondeterministic_render(base_dot, root, rel, "true", before_true.xdot) or nondeterministic_render(dot, root, rel, "true", after_true.xdot):
        ...
        if explicit_concentrate_true(root, rel):
            ...
            if comparable(default_render, false_render) and normalize_xdot_for_default_false(default_render.xdot) != normalize_xdot_for_default_false(false_render.xdot):
                if nondeterministic_render(dot, root, rel, "default", default_render.xdot) or nondeterministic_render(dot, root, rel, "false", false_render.xdot):
    ...
    base_overlap = target_overlap_area(base_target) if base_target else math.nan
    current_overlap = target_overlap_area(current_target) if current_target else math.nan
    ...
    print(f"default_false_xdot_differences={default_false_diffs}")
    ...
    for title, rows in (
        ...
        ("default/false xdot diff sample", default_false_diff_rows[:20]),
