...
All graphics output is handled by the {\tt textspan}, {\tt ellipse},
...
void (*textspan) (GVJ\_t*, pointf, textspan\_t*); \\
...
stored in UTF-8,
a calculated size of the string {\tt txt->size} and
the horizontal alignment {\tt txt->just} of the string in relation to {\tt p}.
Font information is grouped in the {\tt txt->font} field. In particular,
{\tt txt->font->name} and {\tt txt->font->size} give the desired
font name and font size, the latter in points, when a font is specified.
...
{\tt p.x + {\em j} * txt->size.x}, where {\em j} is 0.0 (-0.5,-1.0)
...
gvcolor\_t stopcolor; \\
int gradient\_angle; \\
double gradient\_frac; \\
pen\_type pen;   \\
double penwidth; \\
char *url; \\
char *tailurl;  \\
...
closed regions.
\item[{\tt stopcolor}, {\tt gradient\_angle}, and {\tt gradient\_frac}]
When the current object is filled with a gradient rather than a single
solid color, {\tt fillcolor} and {\tt stopcolor} hold the gradient's
endpoint colors. The {\tt gradient\_angle} field is the angle, in degrees,
used for linear gradients. The {\tt gradient\_frac} field records the
optional color-stop fraction from a {\tt colorList}; a value of 0 means
that no explicit fraction was supplied.
\end{description}
Note that font information is delivered as part of the 
{\tt textspan\_t} value passed to the {\tt textspan} function.
