...
The {\tt dir} attribute determines which end or ends are drawn with arrowheads;
when an end is drawn, {\tt arrowtail} or {\tt arrowhead} gives the requested
shape name, and {\tt arrowsize} gives the scale. Applications can read these
edge attributes with the usual attribute functions such as {\tt agget}. The
{\tt bezier} fields described above give the computed attachment geometry, but
they do not contain a separate decoded copy of the arrow shape grammar.
Applications that need the exact drawing operations Graphviz generated for an
arrowhead, rather than the input attribute value plus the endpoint geometry,
should use the {\tt xdot} drawing attributes: {\tt \_tdraw\_} for the tail
arrowhead and {\tt \_hdraw\_} for the head arrowhead.
