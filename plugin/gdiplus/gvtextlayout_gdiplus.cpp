...
Layout::Layout(char *fontname, double fontsize, char *string) {
  ...
  /* search for a font with this name. if we can't find it, use the generic
   * serif instead */
  /* NOTE: GDI font search is more comprehensive than GDI+ and will look for
   * variants e.g. Arial Bold */
  ...
  LOGFONTA found_font;
  if (EnumFontFamiliesExA(reference.hdc, &font_to_find, fetch_first_font,
                          (LPARAM)&found_font, 0) == 0) {
    found_font.lfHeight = (LONG)-fontsize;
    found_font.lfWidth = 0;
    font = std::make_unique<Font>(reference.hdc, &found_font);
  } else
    font = std::make_unique<Font>(FontFamily::GenericSerif(),
                                  static_cast<REAL>(fontsize));
}
...
bool gdiplus_textlayout(textspan_t *span, char **) {
  /* ensure GDI+ is started up: since we get called outside of a job, we can't
   * rely on GDI+ startup then */
  UseGdiplus();

  Layout *layout = new Layout(span->font->name, span->font->size, span->str);

  /* measure the text */
  /* NOTE: use TextRenderingHintAntiAlias + GetGenericTypographic to get a
   * layout without extra space at beginning and end */
  RectF boundingBox;
  DeviceContext deviceContext;
  Graphics measureGraphics(deviceContext.hdc);
  measureGraphics.SetTextRenderingHint(TextRenderingHintAntiAlias);
  measureGraphics.MeasureString(&layout->text[0], layout->text.size(),
                                layout->font.get(), PointF(0.0f, 0.0f),
                                GetGenericTypographic(), &boundingBox);
  ...
  span->yoffset_layout =
      fontFamily.GetCellAscent(style) * span->font->size /
      fontFamily.GetEmHeight(style); /* convert design units to pixels */
  span->yoffset_centerline = 0;
  return true;
};

static gvtextlayout_engine_t gdiplus_textlayout_engine = {gdiplus_textlayout};
