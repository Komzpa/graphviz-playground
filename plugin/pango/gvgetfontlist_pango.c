...
char *strcasestr(const char *str, const char *pat) {
  ...
  p0 = toupper(p0);
  do {
    while (str <= endp && p0 != toupper(*str))
      str++;
    if (str > endp)
      return NULL;
    pp = pat;
    sp = ++str;
    while ((pc = *pp++) && toupper(pc) == toupper(*sp))
      sp++;
  } while (pc);
  return (char *)(str - 1);
}
...
/* This is where the hierarchy of equivalent fonts is established. The order can
   be changed here or new equivalent fonts can be added here. Each font family
   used by the Graphviz PS fonts is set up.
*/
static const char *PS_AVANT_E[] = {SAN_1, SAN_2, SAN_3, SAN_4, SAN_5,
                                   SAN_6, SAN_7, SAN_8, SAN_9, SAN_10};
#define PS_AVANT_E_SZ (sizeof(PS_AVANT_E) / sizeof(char *))

static const char *PS_BOOKMAN_E[] = {SER_1, SER_2, SER_3, SER_4, SER_5,
                                     SER_6, SER_7, SER_8, SER_9};
#define PS_BOOKMAN_E_SZ (sizeof(PS_BOOKMAN_E) / sizeof(char *))

static const char *PS_COURIER_E[] = {MON_1, MON_2, MON_3, MON_4,
                                     MON_5, MON_6, MON_7, MON_8};
#define PS_COURIER_E_SZ (sizeof(PS_COURIER_E) / sizeof(char *))

static const char *PS_HELVETICA_E[] = {SAN_3, SAN_11, SAN_4, SAN_6,
                                       SAN_7, SAN_8,  SAN_9, SAN_10};
#define PS_HELVETICA_E_SZ (sizeof(PS_HELVETICA_E) / sizeof(char *))

static const char *PS_NEWCENT_E[] = {SER_10, SER_2, SER_3, SER_4, SER_12,
                                     SER_5,  SER_6, SER_7, SER_8, SER_9};
#define PS_NEWCENT_E_SZ (sizeof(PS_NEWCENT_E) / sizeof(char *))

static const char *PS_PALATINO_E[] = {SER_13, SER_2,  SER_3, SER_4,
                                      SER_14, SER_15, SER_5, SER_6,
                                      SER_7,  SER_8,  SER_9};
#define PS_PALATINO_E_SZ (sizeof(PS_PALATINO_E) / sizeof(char *))

static const char *PS_TIMES_E[] = {SER_4, SER_2, SER_11, SER_5,
                                   SER_6, SER_7, SER_8,  SER_9};
...
static const char *PS_CHANCERY_E[] = {SER_16, SER_11, SER_2, SER_3, SER_4,
                                      SER_5,  SER_6,  SER_7, SER_8, SER_9};
...
static int get_faces(PangoFontFamily *family) {
  ...
  for (i = 0; i < n_faces; i++) {
    ...
    /* if the family face type is one of the known types, logically OR the known
       type value to the available faces integer */
...
static void display_available_fonts(availfonts_t gv_af_p) {
  ...
  /* Displays the Graphviz PS font name, system available font name and
   * associated faces */
...
/* This function creates an array of font definitions. Each entry corresponds to
   one of the Graphviz PS fonts.  The font definitions contain the generic font
   name and a list of equivalent fonts that can be used in place of the PS font
   if the PS font is not available on the system
*/
...
*/
static char *gv_get_font(availfonts_t gv_af_p, PostscriptAlias *ps_alias,
                         agxbuf *xb, agxbuf *xb2) {
  char *avail_faces;
...
gv_font_map *get_font_mapping(PangoFontMap *fontmap) {
  ...
  availfonts_t gv_af_p =
      gv_get_ps_fontlist(fontmap); // get the available installed fonts
  ...
  for (size_t j = 0; j < ps_fontnames_sz; j++) {
    ...
    gv_fmap[ps_alias->xfig_code].gv_font =
        gv_get_font(gv_af_p, ps_alias, &xb, &xb2);
  }
